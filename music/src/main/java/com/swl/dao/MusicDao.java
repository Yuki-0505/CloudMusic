...
public class MusicDao {
    ...
    public boolean appendMusic(Music music) {
        String sql = "insert into music(music_id,music_name,writer_name,kind,price,plays,file_path,create_time) values(?,?,?,?,?,?,?,?)";
        return DBUtils.save(sql, music.getMusicId(), music.getMusicName(), music.getWriterName(), music.getKind(), music.getPrice(), music.getPlays(), music.getFilePath(), music.getCreateTime());
    }
    ...
    //  通过userId musicId获取音乐列表
    public List<Music> getMusicByUserIdAndMusicId(Integer userId, Integer musicId) {
        ...
        return DBUtils.getList(Music.class, sql, userId, musicId);
    }
